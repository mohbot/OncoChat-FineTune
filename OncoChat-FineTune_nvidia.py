...
BASE_MODEL = "BioMistral/BioMistral-7B"  # HuggingFace path
LORA_R = 16
LORA_ALPHA = 32
LORA_DROPOUT = 0.05
LEARNING_RATE = 2e-4
NUM_EPOCHS = 3
BATCH_SIZE = 2  # Reduced for 7B model memory
GRADIENT_ACCUMULATION_STEPS = 8 # Increased to maintain effective batch size
MAX_SEQ_LENGTH = 512
TRAIN_SPLIT = 0.9
MODEL_OUTPUT_DIR = "oncochat_biomistral"
...
def generate_qa_pairs(drug_name: str, sections: Dict[str, str]) -> List[TrainingSample]:
    ...
    for section_name, section_text in sections.items():
        ...
        answer = truncate_answer(section_text)
    ...
    if sections:
        ...
        for sec in ["Description", "Indications and Usage", "Warnings and Precautions"]:
            if sec in sections:
                summary_parts.append(truncate_answer(sections[sec], 250))
        if summary_parts:
...
def fine_tune_model(
    ...
    tokenizer.padding_side = "right" # Recommended for Mistral/Llama training
    ...
    log.info("Loading 4-bit BioMistral model")
    ...
    # Specific modules for Mistral architecture
    ...
    training_args = TrainingArguments(
        ...
        save_strategy="epoch",
        eval_strategy="epoch",
        bf16=True, # Optimal for RTX 30-series/40-series
        push_to_hub=False,
        report_to="none",
        gradient_checkpointing=True, # Crucial for 7B models on 24GB VRAM
    )
...
class OncoChat:
    def __init__(self, model_dir: str = MODEL_OUTPUT_DIR, base_model: str = BASE_MODEL):
        log.info("Loading quantized base for inference")
        bnb_config = BitsAndBytesConfig(
            load_in_4bit=True,
    ...
    def generate(self, question: str, max_new_tokens: int = 300) -> str:
        # Standard Instruction prompt format
...
Options:
  ...
  --base-model MODEL  Base model name (default: BioMistral-7B)
